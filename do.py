...
USE_L1S_MODELS_DIR = None

# supported values - branch name or None
USE_OPENAPIART_BRANCH = None
USE_L1S_MODELS_BRANCH = None

OPENAPIART_REPO = "https://github.com/open-traffic-generator/openapiart.git"
L1S_MODELS_REPO = "https://github.com/open-traffic-generator/l1s_models.git"
OPENAPI_YAML_URL = "https://github.com/open-traffic-generator/l1s_models/releases/download/v{}/openapi.yaml".format(
    l1s_models_version
)
...
def generate_sdk():
    ...
    print("handle l1s models dependency")
    if USE_L1S_MODELS_DIR is not None:
        API_FILES = [
            os.path.join(USE_L1S_MODELS_DIR, "api", "info.yaml"),
            os.path.join(USE_L1S_MODELS_DIR, "api", "api.yaml"),
        ]
    elif USE_L1S_MODELS_BRANCH is not None:
        local_path = "testbed_models"
        if not os.path.exists(local_path):
            subprocess.check_call(
                "git clone {} && cd {} && git checkout {} && cd ..".format(
                    L1S_MODELS_REPO, local_path, USE_L1S_MODELS_BRANCH
                ),
                shell=True,
            )
        API_FILES = [
    ...
    openapiart.OpenApiArt(
        ...
        package_dir="github.com/open-traffic-generator/openl1s/%s" % go_pkg_name,
